...
namespace Account.Management.Application.Services
{
    public class VoucherManagementService : IVoucherManagementService
    {
        ...
        public async Task DeleteVoucher(string action, Guid id)
        {
            await _voucherRepository.DeleteAsync(action, id);
        }

        public Task<int> GetTotalVouchersCountAsync(string action)
        {
            return _voucherRepository.GetTotalCountAsync(action);
        }
    }
}
